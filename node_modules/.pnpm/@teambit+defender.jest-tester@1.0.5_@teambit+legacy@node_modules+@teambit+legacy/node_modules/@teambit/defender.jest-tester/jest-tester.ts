...
export class JestTester implements Tester {
  ...
  private attachTestsToComponent(
    ...
    return ComponentMap.as(testerContext.components, (component) => {
      ...
      const resolvedPatterns = this.resolveComponentPattern(
        currComponent,
        patternEntry,
        testerContext
      );
  ...
  private buildTestsObj(
    ...
    const testsSuiteResult = components
      .toArray()
      .map(([component, testsFiles]) => {
        ...
        const tests = testsFiles.map((test) => {
          const file = new AbstractVinyl({
            path: test.testFilePath,
            contents: readFileSync(test.testFilePath),
          });
          ...
          const getError = () => {
            ...
            return new JestError(
              test.testExecError?.message,
              test.testExecError?.stack
            );
          };
  ...
  async test(context: TesterContext): Promise<Tests> {
    ...
    const testsOutPut = await this.jestModule.runCLI(withEnv, [
      this.jestConfig,
    ]);
    const { testResults } = testsOutPut.results;
    const componentsWithTests = this.attachTestsToComponent(
      context,
      testResults
    );
  ...
  async watch(context: TesterContext): Promise<Tests> {
    ...
    return new Promise(async (resolve) => {
      ...
      if (!envRootDir) {
        this.logger.warn(
          `jest tester, envRootDir is not defined, for env ${context.envRuntime.id}`
        );
      }
    ...
    return flatten(
      context.patterns
        ...
        .map(([component, patternEntry]) =>
          this.resolveComponentPattern(component, patternEntry, context)
        )
    );
  }
  ...
  static from(options: JestOptions): EnvHandler<Tester> {
    const id = options.name || 'jest-tester';
    return (context: EnvContext) => {
      const logger = context.createLogger(id);
      const worker = context.createWorker<JestWorker>(
        id,
