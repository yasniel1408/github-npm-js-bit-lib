...
describe('native html link', () => {
    ...
    it('should pass active styles when explicitly active', () => {
        const { getByText } = (0, react_2.render)(react_1.default.createElement(link_1.Link, { href: "/", activeClassName: "active", activeStyle: { fontWeight: 'bold' }, active: true }, "click here"));
    ...
    it('should not pass active styles when explicitly not active', () => {
        const { getByText } = (0, react_2.render)(react_1.default.createElement(link_1.Link, { href: "/", activeClassName: "active", activeStyle: { fontWeight: 'bold' }, active: false }, "click here"));
        const rendered = getByText('click here');
    ...
    it('should automatically pass active style when matching location', () => {
        const { getByText } = (0, react_2.render)(react_1.default.createElement(link_1.Link, { href: "/", activeClassName: "active", activeStyle: { fontWeight: 'bold' } }, "click here"));
        const rendered = getByText('click here');
        expect(rendered).toHaveClass('active');
        expect(rendered).toHaveStyle({ fontWeight: 'bold' });
    });
    it('should automatically skip active style when not matching location', () => {
        const { getByText } = (0, react_2.render)(react_1.default.createElement(link_1.Link, { href: "/other-path", activeClassName: "active", activeStyle: { fontWeight: 'bold' } }, "click here"));
        const rendered = getByText('click here');
        expect(rendered).not.toHaveClass('active');
        expect(rendered).not.toHaveStyle({ fontWeight: 'bold' });
    });
